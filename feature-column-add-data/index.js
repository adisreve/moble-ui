    ...
    // import { base_price, existing_data, options, data } from "./modules/state.js";
    ...
    const applyData = () => {
        ...
        const applyInventory = dataContainer.querySelector('input[name="apply-inventory"]');
        ...
        applyDataSubmit.addEventListener('click', (e) => {
            ...
            // Instanciate datatables api
            const table = $('#tab_logic').dataTable().api();
            const tableColumns = table.columns().data().length;

            const columns = {
                sku: {
                    value: applySku.value,
                    column: tableColumns - 3
                },
                inventory: {
                    value: applyInventory.value,
                    column: tableColumns- 2
                },
                price: {
                    value: applyPrice.value,
                    column: tableColumns - 1
                }
            }

            // Get current table columns based on results
            const results = table.columns( { search: 'applied' } ).data();
            
            // Get all keys from columns objec
            ...
            console.log(applySku.value, applyInventory.value, applyPrice.value);
        })
